...
{
	// usd every bit to test fbits. write info to buf.featureset? every bit to a bit.
    ...
    *buf = '\0';
    for (q = buf, bit = 31; fbits && bufsize; --p, fbits &= ~(1 << bit), --bit)
        if (fbits & 1 << bit && (*p || !flags)) {
        	// if in this block.then p must be val.
            if (*p)
...
{
	// convert builtin_x86_defs from array to list
    int i;
