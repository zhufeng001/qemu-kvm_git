        ...
        if (f) {
            do {
                ...
                if (n == 2) {
                    ...
                    endaddr = MIN(endaddr,
                                    (1ULL << TARGET_PHYS_ADDR_SPACE_BITS) - 1);
                    // start high bits; end high bits ;0x0020
...
{
	// is index is addr & pagesize
    PageDesc **lp, *p;
    // l1 is high.and l2 is low
...
{
	// code_gen_buffer (map_exec)
	// code_gen_buffer mmap(malloc) code_gen_buffer_size
	// code_gen_prologue map_exec
...
{
	// init env and insert to first_env
    CPUState **penv;
    int cpu_index;
    ...
    penv = &first_cpu;
    cpu_index = 0;
    // get by cpu_index by Traversal penv
    while (*penv != NULL) {
        penv = &(*penv)->next_cpu;
        cpu_index++;
    }
...
static inline void tb_remove(TranslationBlock **ptb, TranslationBlock *tb,
                             int next_offset)
{
	// next_offset point to next tb. ptb is from (last tb1)->next_offset.
    TranslationBlock *tb1;
    for(;;) {
        tb1 = *ptb;
        if (tb1 == tb) {
        	// let (last tb1)->next_offset point to (now tb1)->next_offset
...
static inline void tb_page_remove(TranslationBlock **ptb, TranslationBlock *tb)
{
    TranslationBlock *tb1;
    unsigned int n1;
    // tb1记录两个信息。一个是 2位bit。还有一个是指针tb1
    // tb1 record 2 info .2bit into and pointer info.
    //
    for(;;) {
        tb1 = *ptb;
        n1 = (long)tb1 & 3;
    ...
    if (tb1) {
        ...
        for(;;) {
            ...
            if (n1 == n && tb1 == tb)
                break;
            if (n1 == 2) {
            	// store in cyle. find the first
                ptb = &tb1->jmp_first;
            } else {
            	// not stored all in 1 or 0.
                ptb = &tb1->jmp_next[n1];
            }
        }
...
void tb_phys_invalidate(TranslationBlock *tb, target_ulong page_addr)
{
    ...
    // tb insert into tb_phys_hash by hash
    tb_remove(&tb_phys_hash[h], tb,
              offsetof(TranslationBlock, phys_hash_next));

    /* remove the TB from the page list */
    // remove tb from first_tb
    if (tb->page_addr[0] != page_addr) {
    	// get tb->page_addr[0]'s l2 in l1
    ...
    tb1 = tb->jmp_first;
    for(;;) {
    	// remove every elm in jmp_first
        n1 = (long)tb1 & 3;
        if (n1 == 2)
        	// walk a cycle and stop
            break;
        tb1 = (TranslationBlock *)((long)tb1 & ~3);
        tb2 = tb1->jmp_next[n1];
...
static void build_page_bitmap(PageDesc *p)
{
    ...
    tb = p->first_tb;
    while (tb != NULL) {
    	// get tb start and end .and set code_bitmap in p
        n = (long)tb & 3;
...
{
	// reset env->tlb_table , tb_jmp_cache
    int i;
...
{
    RAMBlock *block;
    // use addr get block from ram_list and return block->host+offset
...
{
    int i;
    // get io_index from io_mem_used
    // set io_mem_read,io_mem_wrte and io_mem_opaque
    // why use IO_MEM_ROM,and then >> ??
...
void cpu_physical_memory_rw(target_phys_addr_t addr, uint8_t *buf,
                            int len, int is_write)
{
	// write to buf or read from buf write to addr according to id_write
    int l, io_index;
    uint8_t *ptr;
    uint32_t val;
    ...
    while (len > 0) {
        ...
        // buf store val
        if (is_write) {
        	// get val from buf .write to addr1
            if ((pd & ~TARGET_PAGE_MASK) != IO_MEM_RAM) {
            	// not in IO_MEM_RAM
            	// get io_index by pd
		...
		if (kvm_enabled())
        	...
        	// get val from addr1 ,write to buf
            if ((pd & ~TARGET_PAGE_MASK) > IO_MEM_ROM &&
                !(pd & IO_MEM_ROMD)) {
                target_phys_addr_t addr1 = addr;
                /* I/O case */
...
{
	// read val from addr
	// use addr to another addr (phys addr?)
    int io_index;
    uint8_t *ptr;
    uint64_t val;
    unsigned long pd;
    PhysPageDesc *p;
    // find l2 in l1 by addr
    p = phys_page_find(addr >> TARGET_PAGE_BITS);
    if (!p) {
        pd = IO_MEM_UNASSIGNED;
    } else {
    ...
    if ((pd & ~TARGET_PAGE_MASK) > IO_MEM_ROM &&
        ...
        io_index = (pd >> IO_MEM_SHIFT) & (IO_MEM_NB_ENTRIES - 1);
        if (p)
        	// re calcutlation addr
            addr = (addr & ~TARGET_PAGE_MASK) + p->region_offset;
#ifdef TARGET_WORDS_BIGENDIAN
        //  get val by io_mem_read high 32 bit use addr 0 1 2 3 4
        val = (uint64_t)io_mem_read[io_index][2](io_mem_opaque[io_index], addr) << 32;
        // get val by io_mem_read low 32bit by addr 4 5 6 7
...
{
	// read from addr to val len is 1
    uint8_t val;
...
void stl_phys(target_phys_addr_t addr, uint32_t val)
{
    ...
    // check pd and get index ,execute io_mem_write with io_index,io_mem_opaque and addr ,val
    if ((pd & ~TARGET_PAGE_MASK) != IO_MEM_RAM) {
        io_index = (pd >> IO_MEM_SHIFT) & (IO_MEM_NB_ENTRIES - 1);
        if (p)
            addr = (addr & ~TARGET_PAGE_MASK) + p->region_offset;
        ...
        addr1 = (pd & TARGET_PAGE_MASK) + (addr & ~TARGET_PAGE_MASK);
        /* RAM case */
        // get addr1 from ram_list
        ptr = qemu_get_ram_ptr(addr1);
        // set ptr point to val
        stl_p(ptr, val);
        // no data
        if (!cpu_physical_memory_is_dirty(addr1)) {
            /* invalidate code */
            tb_invalidate_phys_page_range(addr1, addr1 + 4, 0);
            /* set dirty bit */
            // if addr1 phys_dirty not contain  MIGRATION_DIRTY_FLAG
            if (!cpu_physical_memory_get_dirty(addr1, MIGRATION_DIRTY_FLAG))
                ram_list.dirty_pages++;
            // set phys_dirty by addr1
            ram_list.phys_dirty[addr1 >> TARGET_PAGE_BITS] |=
                (0xff & ~CODE_DIRTY_FLAG);
        }
    }
...
{
	// use addr to get index and use io_mem_wirte to write val
    int io_index;
    uint8_t *ptr;
    unsigned long pd;
    PhysPageDesc *p;
